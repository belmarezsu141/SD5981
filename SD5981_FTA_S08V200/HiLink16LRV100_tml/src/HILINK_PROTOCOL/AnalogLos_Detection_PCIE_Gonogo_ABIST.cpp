 ...
 * class is created.
 */
class AnalogLos_Detection_PCIE_Gonogo_ABIST: public TestClass {
public:
...
//			INT					iTxMainStep = 3;
			...
			hout(DETAIL) << "The current TxFIR Main For PCIE ALOS = " << FIR.iTxMain << endl;
			d2s::d2s_LABEL_BEGIN("H16LRTV100_MPB_RXALOS_PCIE_ABIST_HIGH", Global_D2S_Mode);
			...
			d2s::d2s_LABEL_BEGIN("H16LRTV100_MPB_ANALOG_LOS_PCIE_CheckHIGH",Global_D2S_Mode);
...
//			if (jtag.hasAllSitesPassed("H16LRTV100_MPB_ANALOG_LOS_PCIE_CheckHIGH")) {
			if (hasAllSitesPassed()) {
				TxFir_Main_High_IsNoAlos.init(1);
				hout(DETAIL) << "indicate LOS on HIGH!" << endl;
			} else {
			...
			d2s::d2s_LABEL_BEGIN("H16LRTV100_MPB_RXALOS_PCIE_ABIST_LOW", Global_D2S_Mode);
			...
			d2s::d2s_LABEL_BEGIN("H16LRTV100_MPB_ANALOG_LOS_PCIE_CheckLOW",Global_D2S_Mode);
...
//			if (jtag.hasAllSitesPassed("H16LRTV100_MPB_ANALOG_LOS_PCIE_CheckLOW")) {
			if (hasAllSitesPassed()) {
			...
			//restore
			//=============================================
			d2s::d2s_LABEL_BEGIN("H16LRTV100_MPB_RXALOS_PCIE_ABIST_RESTORE", Global_D2S_Mode);
		...
		// Datalog
		//=============================================
//		HiLink_PerLane_Judge(MacroLane_Sel, "High_IsNoAlos_PCIE", TxFir_Main_High_IsNoAlos);
//		HiLink_PerLane_Judge(MacroLane_Sel, "Low_IsAlos_PCIE", TxFir_Main_Low_IsAlos);

		return;
	}

};
REGISTER_TESTCLASS("AnalogLos_Detection_PCIE_Gonogo_ABIST", AnalogLos_Detection_PCIE_Gonogo_ABIST);
