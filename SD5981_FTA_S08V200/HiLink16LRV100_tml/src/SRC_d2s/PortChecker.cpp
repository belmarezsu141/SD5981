...
//        //cout<<"portName = "<<portName<<"\ti = "<<i<<endl;
...
//#if DEBUG
//            	cout<<"portAName = "<<portAName<<";"<<"portBName = "<<portBName<<endl;
//            	cout<<"portAIndex = "<<portAIndex<<";"<<"portBIndex = "<<portBIndex<<endl;
//#endif
...
bool PortChecker::arePortsIntersecting(const std::set<std::string> &portNames){
    ...
    for(std::set<std::string>::const_iterator portA_it=portNames.begin(); portA_it!=portNames.end(); portA_it++){
        ...
        for(std::set<std::string>::const_iterator portB_it=tmpIt; portB_it!=portNames.end(); portB_it++){
        	if(portIndex.find(*portA_it)==portIndex.end()){
//        		cout<<"Error: port '"<<*portA_it<<"' is not exists!"<<endl;
        		assert(0);
        	}
        	if(portIndex.find(*portB_it)==portIndex.end()){
//        		cout<<"Error: port '"<<*portB_it<<"' is not exists!"<<endl;
        		assert(0);
        	}
            if(intersectingPortTable[portIndex[*portA_it]][portIndex[*portB_it]] == 1){
//            	cout<<"Error: port '"<<*portA_it<<"' and port '"<<*portB_it<<"' is intersect!"<<endl;
            	portsDoIntersect = true;
            }
        }
    }
