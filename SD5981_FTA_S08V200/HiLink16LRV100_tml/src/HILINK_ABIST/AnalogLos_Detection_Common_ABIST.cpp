 ...
 * class is created.
 */
class AnalogLos_Detection_Common_ABIST: public TestClass {
public:
	...
	void execute()
		...
		static LANE_DOUBLE			All1_MaxSwing;
			...
			while ((FIR.iTxMain >= iTxMainStop) && ((iFind_All == 0) || (Global_D2S_Mode == LearningMode)))
			{
				...
				if (isAll0Flag) {
					d2s::d2s_LABEL_BEGIN("H16LRTV100_MPB_ANALOG_LOS_COMMON_CheckAll0",Global_D2S_Mode);
...
//					if (jtag.hasAllSitesPassed("H16LRTV100_MPB_ANALOG_LOS_COMMON_CheckAll0")) {
...
REGISTER_TESTCLASS("AnalogLos_Detection_Common_ABIST", AnalogLos_Detection_Common_ABIST);
