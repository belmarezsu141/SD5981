 ...
 * class is created.
 */
class TxIjEScopePrbs15_ABIST: public TestClass {
...
//		add_param("PinList","PinString",&mPinList);
	...
	// PRBS15 pattern under internal loopback mode, meanwhile Tx inject jitter (  //
	// ~0.5UI@datarate/1667). The test can perform under the setups include       //
	// different data rate and different bit width.                               //
	//																			  //
	// Parameters:																  //
	...
	void execute()
		...
		static	LANE_DOUBLE 		Eyeheight;
			...
			JitterAmpRequired = 1;/*UI*/

			d2s_JTAG& jtag = d2s_JTAG::Instance();

			sprintf(label,"H16LRTV100_MPB_TXIJESCOPEPRBS15_SETUP_RateMode%d_DataWidth%d",CUSTpara.RateMode,CUSTpara.DataWidth);
			d2s::d2s_LABEL_BEGIN(label,Global_D2S_Mode);
				...
				JI 	JITINDEX = JitterInjection(CUSTpara.DataWidth, CUSTpara.RateMode, CUSTpara.Data_Rate, JitterAmpRequired);
			...
			sprintf(label,"H16LRTV100_MPB_TXJIESCOPEPRBS15_RESTORE_CDR_RateMode%d_DataWidth%d",CUSTpara.RateMode,CUSTpara.DataWidth);
			d2s::d2s_LABEL_BEGIN(label,Global_D2S_Mode);
			...
			FOREACH_ACTIVESITE_BEGIN(SiteID, bInterrupt)
				...
				for (UINT macroIndex = 0; macroIndex < MacroLane_Sel.size(); macroIndex ++)
				{
					...
					for (UINT laneIndex = 0; laneIndex < vLane.size(); laneIndex ++)
					{
						...
						if (iOffline) {
							Jitter[SiteID][MacroID][LaneID] = 1;
							Eyeheight[SiteID][MacroID][LaneID] = 3;
						}
					}
				}
...
REGISTER_TESTCLASS("TxIjEScopePrbs15_ABIST", TxIjEScopePrbs15_ABIST);
