...
void d2s::write(long long llAddress, long long llData, long long llMd5Data) {
	//add for H30X
	if (((llAddress<<1) >= 0x19800) &&  ((llAddress<<1) <= 0x1FFFF)){
		llAddress = ((llAddress<<1) - 0x6800)>>1;
	}
	//end
...
//		cout<<"d2s::wait timing block now  static  use TIM_HiLink30LR, not use GetCurrentTimingBlockName() api"<<endl;
		double period = TheSoft.Flow().Timing().GetPeriod("TIM_HiLink30LR", getTimingSetName());
		if(period == 0)
		{
